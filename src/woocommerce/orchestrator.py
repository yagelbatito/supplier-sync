...
class WhatsAppOrchestrator:
    ...
    def handle_incoming_image(
        ...
        self.wa.send_text(
            ...
            f"✅ זוהה:\n{summary}\n\n"
            f"השב להודעה זו עם:\n"
            f"ספק:perlahome מחיר:150\n\n"
            f"(חסר: {missing}. אפשר להוסיף גם: מקט:XXX, שם:..., קטגוריה:...)",
            reply_to_msg_id=message_id,
        )
    ...
    def handle_incoming_text(
        ...
        if cmd.name_override:
            ocr_result.name = cmd.name_override
        if cmd.sku_override:
            ocr_result.sku = cmd.sku_override
        if cmd.category_override:
            ocr_result.category_hint = cmd.category_override
    ...
    def _run_sync(
        ...
        config = self._get_supplier_config(cmd.supplier)
        if not config:
            self.wa.send_text(
                from_number,
                f"❌ ספק לא ידוע: {cmd.supplier!r}\n"
    ...
    def _build_product(
        ...
        if ocr.confidence == "low" and hasattr(product, "mark_for_review"):
